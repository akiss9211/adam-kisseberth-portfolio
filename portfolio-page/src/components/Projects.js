...
function Projects() {
 ...
 //state to hold and update current filter string from input
  ...
  const [currency, setCurrency] = useState({ platinum: 0, gold: 0, silver: 0, copper: 0 })
 //gets local storage inventory save and sets inventory state with data
  //by parsing into JS objects
  
 //checks to see if inventory and currency save exists in local storage before retrieving
 useEffect(() => { 
  const localCurrency = localStorage.getItem('currency-items');
 ...
 //sends inventory and currency state data to local storage
 useEffect(() => {
  if(currency.platinum > 0 || currency.gold > 0 || currency.silver > 0 || currency.copper > 0) {
    localStorage.setItem('currency-items', JSON.stringify(currency))
  } else {
    localStorage.clear();
  }
  if(inventory.length) {
